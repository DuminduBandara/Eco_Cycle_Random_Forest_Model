...
def main(start_lat, start_lon, end_lat, end_lon):
    ...
    # Step 4: Fetch Route Coordinates
    ...
    # Step 5: Filter Attractions Within 2 km
    try:
    ...
    if len(unique_y_classes) < 2:
        print("Warning: Only one class present in y. Skipping classifier training.", file=sys.stderr)
        accuracy = 1.0
        df["predicted_relevant"] = y
        classification_metrics = {
            "Relevant": {
                "precision": 1.0 if unique_y_classes[0] == 1 else 0.0,
                "recall": 1.0 if unique_y_classes[0] == 1 else 0.0,
                "f1-score": 1.0 if unique_y_classes[0] == 1 else 0.0
            }
        }
        clf = None
    else:
        ...
        try:
            ...
            generated_files.append("random_forest_model.pkl")
        ...
        classification_metrics = classification_report(y_test, y_pred, output_dict=True, labels=[0, 1], zero_division=0)
    ...
    # Step 7: Generate Ground Truth and Detection Metrics
    ground_truth = []
    for _, row in df.iterrows():
        spot_coord = (row["latitude"], row["longitude"])
        for route_coord in route_coords_list[::5]:
    ...
    # Step 8: Generate HTML Map for Nearby Attractions
    CLUSTER_COLOR = 'blue'
    segment_colors = [
