...
async def predict_tourism(coords: Coordinates):
    try:
        ...
        # Check if model file exists with enhanced error message
        model_path = get_file_path("random_forest_model.pkl")
        if not os.path.exists(model_path):
            raise HTTPException(
                status_code=404,
                detail="Model file 'random_forest_model.pkl' not found. "
                       "Please ensure the model has been generated by running 'train_tourism_classifier.py' "
                       "and that the file is located in the project directory."
            )
